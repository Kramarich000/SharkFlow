      ...
      <div className="max-w-[1240px] mx-auto flex justify-between items-center">
        ...
        >
          {navLinks(token).map((link) => (
            <Link key={link.path} to={link.path} className="transition-colors">
              {link.label}
            </Link>
          ))}
          <button
            ...
            className={`!w-fit !p-3 !text-[24px] !font-normal !transition-colors ${!token ? 'hidden pointer-events-none select-none' : null}`}
          >
            Выход
          </button>
        </motion.nav>
